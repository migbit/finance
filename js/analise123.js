...
chartTotal = new Chart(document.getElementById('chart-total'), {
 ...
 // ----------------------------------------------------------------------------> BARRAS DE PROGRESSO
// ----------------------------------------------------------------------------> BARRAS DE PROGRESSO (DOUGHNUTS) — Apt 123
...
// plugin center text (safe for any canvas size)
const centerText = {
  id: 'centerText',
...
function makeDonut(canvas, percentSigned) {
  ...
  // keep size sane to avoid huge backing stores
  ...
  const formatted = val.toFixed(2); // ← show 2 decimals
...
// Title: "Progresso vs (ano anterior)"
{
...
// 1) Parcial do mês atual (Apt 123)
{
  ...
  const pct  = (diff / base) * 100;       // keep as float
...
// 2) Até mês anterior (acumulado, Apt 123)
{
...
// 3) Ano atual vs anterior (total até agora, Apt 123)
{
